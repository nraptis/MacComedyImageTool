//
//  ImageTransparencyTrimmer.swift
//  JokeSeedTool
//
...
class ImageTransparencyCropper {
    
    func trimmingTransparentPixels(image: NSImage, maximumAlphaChannel: UInt8 = 0) -> NSImage? {
        guard image.size.height > 1 && image.size.width > 1 else {
            return image
        }
        ...
        let result = NSImage(cgImage: cgImage, size: scaledSize)
        return result
    }
}
